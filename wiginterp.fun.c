 ...
 * int    flag     ----  1 means x is inconsistantly sampled.
 *                      other means x is equally sampled.
 *
 * Shule Yu
 * Mar 27 2014
 *
...
double geteps(double *, int , double );
double geteps_xy(double *, int , double *);
double geteps_f(float *, int , float );
double geteps_xy_f(float *, int , float *);

void wiginterp_f(float *x, float *y, int npts, double *xx, double *yy, int Newnpts, int flag){

    double dx,epsi;
    if (flag==1){
        epsi=geteps_xy_f(y,npts,x);
        dx=0.0;
    }
    else {
        dx=(*(x+1))-(*x);
        epsi=geteps_f(y,npts,dx);
    }
...
void wiginterpd(double *x, double *y, int npts, double *xx, double *yy, int Newnpts, int flag){
    ...
    double dx,epsi;
    if (flag==1){
        epsi=geteps_xy(y,npts,x);
        dx=0.0;
    }
    else {
        dx=(*(x+1))-(*x);
        epsi=geteps(y,npts,dx);
    }
...
double geteps_f(float *y, int npts, float del) {
...
double geteps_xy_f(float *y, int npts, float *x) {
...
double geteps(double *y, int npts, double del) {
...
double geteps_xy(double *y, int npts, double *x) {
    double EPS=0;
    int    count;
